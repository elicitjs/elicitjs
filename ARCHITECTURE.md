...
1. **Core engine (`elicit.Elicit`)** — the orchestrator (`src/core/elicit.js`). Deep-copies the spec's **one dataset** into a reactive store, resolves one **global scale per channel** (Observable-Plot model), rebuilds the scene each render, and routes gesture events to the matching edits. The unidirectional flow: `gesture → invert through scale → data-space proposal → data invariants → commit → re-render`.
   ...
   **One schema.** The schema is the contract of the elicited dataset: every field's measurement type and domain. It is what lets a chart resolve its scales and mint rows from *zero* starter data. A scale is one **encoding**: a `(channel, field)` pair. Two marks stating the same encoding share one scale, so a shared colour means a shared value; two marks putting *different* fields on `fill` are two encodings, so they get two scales, two colour families and two legends. **Positional** channels are the exception and union by axis — an error bar's `mean`, `lo` and `hi` share one y axis that spans all three, which is the intent.
...
3. **Marks (`elicit.plot.*`)** — pure data-to-geometry factories on a shared foundation (`src/plot/mark.js`): every mark resolves its channels through `encodeChannel` and the standard style surface (`fill`, `stroke`, `strokeWidth`, `opacity`, …) the same way. Marks compose across scale types and orientations.
   ...
   - `line` / `lineY` / `lineX` / `path` → a connecting `path` per series plus one `circle` handle per datum; grouped by `series`, ordered by `order`.
   ...
   - `arc` / `donut` → stacked angular slices (part-to-whole). Magnitudes on the `theta` channel's field normalize to a full or partial circle; `innerRadius` makes a donut. (The magnitude channel is `theta`, the polar POSITION, read raw and normalized by the mark; `angle` means a rotation in place. There is no `pie`: a full pie IS `arc()`.) Pass `edits: [edit.stack.cut(), edit.stack.edge(), edit.stack.merge()]` to cut a slice in two, drag a boundary, or merge two back together (see `edit.stack.*` below) — a pie divides a total the same way a stacked bar does.
...
3b. **Chart elements (`elicit.elements.*`)** — scale chrome, not data marks. They view a SCALE (`views: 'scale'`), take a singular `channel`, paint CHROME (not desugared channels), and their edits target the schema DOMAIN (`edit.axis.*`). See the `ChartElement` interface in `src/types.d.ts`.
   ...
   - `legend` / `legendColor` / `legendSize` / `legendSymbol` → a key for one ENCODING
     (a `(channel, field)` pair), which may reserve layout space. Usually you do not
     compose one: declare it where the encoding is (`fill: { field, legend: true }`) or
     chart-wide with `legends: true`, and `core/legends.js` builds it. Compose one to
     place it yourself, or to key an encoding a preset declared.
...
4. **Edits (`elicit.edit.*`)** — a gesture that writes a channel back to the data. An edit is a small descriptor `{ gesture, channels, when, pick, scope, constrain, guide, apply }`, declared **co-located** on a channel (`channels.y.edit = move()`) or at **mark level** (`edits: [...]`).
   - **Universal** edits (any mark): `move`, `moveSpan` / `brushSpan` (move / edge-resize a 1-D span), `brushRect` (composable 2-D edge/corner/body editing of a rect's four extents), `resize`, `rotate` (pointer angle about the plot centre — or `pivot: 'mark'` — → a channel value; `fold: false` for full-circle dials; `pick: 'direct'` for a needle handle), `cycle`, `create`, `toggle` (click a slot to pick or un-pick it), `remove`, `editText` (retype a text mark's content), `rank` (drag to reorder a ranked slot), `select`, `set`, `custom`.
   ...
   - **Scale- and axis-scoped** edits — the one family that writes the **schema's domain**, not a datum (they carry `target: 'domain'`). `edit.axis.scale()` drags a numeric/temporal axis's end-handle to grow/shrink its range (`mode: 'grow'` resizes the chart instead of rescaling in place); it is axis-scoped because a positional RANGE is an axis's alone. `edit.scale.categories()` adds / renames / removes categories on any element drawing a discrete scale — an axis or a legend, since the domain belongs to the scale rather than to the chrome (reusing the `editText` inline-typing lifecycle; rename relabels matching rows and remove deletes them, across every column on the scale and every table it spans; `mode: 'grow'` grows the chart by one band-step per category instead of re-dividing it — e.g. extending a 5-point Likert scale to 7). The domain lives on the schema and scales re-resolve every render, so the grid, guides and marks reflow for free. Read the reshaped domain with `el.getSchema()`.
   ...
   - **Scope goes in the name.** A namespaced edit (`edit.line.*`, `edit.stack.*`, `edit.waffle.*`, `edit.geo.*`, `edit.axis.*`, `edit.scale.*`, `edit.legend.*`, `edit.trend.*`, `edit.network.*`) needs the matching mark family, and each declares a `scope` naming the mark capability it requires. Attach one to a mark that lacks the capability and the engine dev-warns rather than leaving you with a gesture that silently does nothing.
   ...
   - `when` arbitrates when several edits share a gesture (`edit.when.alt`, `noAlt`, `shift`, `near`, `far`, …): e.g. plain click recolours, Alt-click deletes.
...
5. **Constraints (`elicit.constraints.*`)** — **data-layer invariants**: pure rules over the dataset, run on every edit commit (never see pixels). They both *gate* a proposal (return `false` to reject) and *repair* it (return the corrected rows) — and since the rows are shared, a repair propagates to every mark on the next render. Declared once on the spec (`constraints: [...]`) — the only place, because the rule is about the data and holds for **every** edit from **every** mark, so writing it inside one mark would claim a scope it does not have. Per-edit sugar is `edit.constrain`. Built-ins fall into three kinds — **bounds**: `clamp({ min, max, field })`, `snap({ field, step, origin })`; **cardinality**: `count({ max, strategy })`, `unique({ field, max })`, `maintainSum({ total, field })`; and **shape**: `ordering({ field: [...] })` keeps fields of a row in order (`lo <= mean <= hi`, so an interval glyph can't be dragged inside-out), `monotonic({ field, along, dir })` stops a curve reversing along an axis (a CDF that dips means negative probability mass), `spacing({ field, min })` keeps adjacent values a minimum distance apart. The shape rules **repair by pushing the neighbours aside**, holding the field you actually dragged — they know which one that is by diffing against the previous rows — so a crossed handle reads as the interval moving rather than as the handle sticking (`ordering`'s `strategy: 'block'` rejects instead). A constraint is a DESCRIPTOR — `{ type, field?, options, apply }`, the parallel of an edit's — and its `field` defaults to the column the dispatching edit writes, so `clamp({ min: 0 })` on a one-column instrument names nothing twice. Author your own with `constraints.custom(apply, meta?)` — write just the rule against a clean data context (`{ data, oldData, activeIndex, active, field, value, domain, table, tables }`) and return a number (set the field), object (merge), array (replace dataset), or `false` (reject).
...
**Reading data out.** `Elicit(spec)` returns the chart element augmented with a small observation API: `getData()` (a deep copy of the committed belief dataset), `getSchema()` (a deep copy of the engine-owned schema, including any domain an editable axis reshaped — the caller's `spec.schema` is never mutated), `setData(data)` (seed/reset + re-render; also clears the undo history, since a reseed is a new starting point rather than an edit), and `on("change" | "stage" | "select", cb)` (subscribe; returns an unsubscribe). This is in addition to the spec's `onChange`.

**Selection.** Transient pipeline state, not belief data, with its own API: `getSelection()` / `getSelectionAll()`, `select(index | index[] | null)`, `toggleSelection(index)`, `selectAll()`, `selectWhere(field, value, { all })` / `selectWhere(predicate)`, `clearSelection()`; every route goes through one commit, and the `'select'` event reports it.

**External control.** `el.emit(event)` injects a renderer-shaped gesture; `el.control(name, index)` returns a handle bound to the edit named `name` (`move({ name })`) with `.set(value)`, `.begin()`/`.end()`, `.fire(gesture?)`, `.emit(event)` and `.accepts()` — a new input SOURCE, not a second interaction system, since it feeds the one dispatch.
...
│   ├── renderers/
│   │   ├── d3-renderer/    # The default SVG renderer
│   │   └── canvas/         # The canvas-2D renderer (same Renderer contract)
...
Public packaging:
- `elicit.plot.*` — data marks. Every name is a keyword: the `…X`/`…Y` pairs are the bare mark with `orientation` pinned
